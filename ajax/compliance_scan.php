...
// 调试日志 - 记录请求开始
error_log("COMPLIANCE_SCAN AJAX: Request received at " . date('Y-m-d H:i:s'));
error_log("COMPLIANCE_SCAN AJAX: POST data = " . json_encode($_POST));
error_log("COMPLIANCE_SCAN AJAX: Session data = " . json_encode(array_keys($_SESSION ?? [])));

include('../../../inc/includes.php');

// 调试日志 - 确认include成功
error_log("COMPLIANCE_SCAN AJAX: After includes.php - User ID: " . (Session::getLoginUserID() ?? 'none'));
    ...
    if ($user_group_check_needed) {
        ...
        if (!empty($rule['groups_id'])) {
            ...
            if (is_array($group_ids)) {
                ...
                $result = $DB->doQuery($computer_group_query);
                if ($result) {
...
try {
    ...
    // 注意: Session::checkCSRF() 会自动检查 $_POST['_glpi_csrf_token']
    ...
    if (!Session::haveRight('config', UPDATE)) {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Permission denied: Requires config UPDATE access'
        ]);
        exit;
    }
    ...
    // 步骤5: 创建扫描历史记录（使用 GLPI 11.x 兼容的 insert 方法）
