<?php
/**
 * AJAX Whitelist Form Submission Endpoint
...
try {
    ...
    if ($result_id) {
        echo json_encode([
            ...
            'id' => $result_id,
            'action' => $action,
            'redirect' => $CFG_GLPI['root_doc'] . '/plugins/softwaremanager/front/whitelist.php'
        ]);
    } else {
    ...
    error_log('AJAX whitelist submit error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    echo json_encode(['success' => false, 'error' => $e->getMessage()]);
}
