...
function exportData($listType) {
    ...
    // 直接查询数据库
    $rows = [];
    foreach ($DB->request(['FROM' => $tableName, 'WHERE' => ['is_deleted' => 0], 'ORDER' => 'name ASC']) as $row) {
        $rows[] = $row;
    }
...
function convertIdsToNames($json_ids, $type) {
    ...
    foreach ($valid_ids as $id) {
        ...
        if ($id > 0) {
            ...
            if ($type === 'groups') {
                // 群组表没有is_deleted字段
                foreach ($DB->request(['SELECT' => ['name'], 'FROM' => $table, 'WHERE' => ['id' => $id], 'LIMIT' => 1]) as $row) {
                    $names[] = $row['name'];
                }
            } elseif ($type === 'users') {
                // 用户表返回更友好的显示名称
                foreach ($DB->request(['SELECT' => ['name', 'realname', 'firstname'], 'FROM' => $table, 'WHERE' => ['id' => $id, 'is_deleted' => 0], 'LIMIT' => 1]) as $row) {
                ...
                // 计算机表
                foreach ($DB->request(['SELECT' => ['name'], 'FROM' => $table, 'WHERE' => ['id' => $id, 'is_deleted' => 0], 'LIMIT' => 1]) as $row) {
                    $names[] = $row['name'];
                }
            }
        }
...
function debugData() {
    ...
    // 查询黑名单数据
    $rows = [];
    $result = $DB->doQuery("SELECT id, name, computers_id, users_id, groups_id FROM glpi_plugin_softwaremanager_blacklists WHERE is_deleted = 0 AND (computers_id IS NOT NULL OR users_id IS NOT NULL OR groups_id IS NOT NULL) LIMIT 3");
    if ($result) {
    ...
    if (!empty($rows)) {
        ...
        // 查看是否有任何记录
        foreach ($DB->request(['COUNT' => 'cpt', 'FROM' => 'glpi_plugin_softwaremanager_blacklists', 'WHERE' => ['is_deleted' => 0]]) as $row) {
            echo "<br>总记录数: " . $row['cpt'];
            break;
        }
    }
}
