<?php
/**
 * Secure Real Import Handler for Software Manager Plugin
...
error_log("=== Secure Real Import Handler Started ===");
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $response = [
            'success' => true,
            'message' => 'Secure real import handler ready',
            'status' => 'ready',
            'authenticated' => true,
        ...
        foreach ($csv_data as $row_index => $row) {
            ...
            try {
                // Prepare data with name-to-ID conversion for related fields
                $input = [
                ...
                // Convert names to IDs for enhanced fields
                $input['computers_id'] = convertNamesToIds(trim($row[6] ?? ''), 'computers');
                $input['users_id'] = convertNamesToIds(trim($row[7] ?? ''), 'users');
                $input['groups_id'] = convertNamesToIds(trim($row[8] ?? ''), 'groups');

                // Set related requirements based on whether IDs were provided
                if (!empty($input['computers_id'])) {
                    $input['computer_required'] = 1;
                }
                if (!empty($input['users_id'])) {
                    $input['user_required'] = 1;
                }
                if (!empty($input['groups_id'])) {
                    $input['group_required'] = 1;
                }

                // Version rules
                if (!empty(trim($row[9] ?? ''))) {
                    $input['version_required'] = 1;
                    $input['version_rules'] = trim($row[9]);
                }
        ...
        $response = [
            'success' => true,
            'message' => "Real import completed: $success_count inserted, $updated_count updated, $error_count failed",
    ...
    error_log("Secure real import handler error: " . $e->getMessage());
...
error_log("=== Secure Real Import Handler Ended ===");
