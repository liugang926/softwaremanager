...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            // 尝试读取GLPI配置文件获取数据库连接信息
            $config_paths = [
                __DIR__ . '/../../../config/config_db.php',
                realpath(__DIR__ . '/../../../config/config_db.php'),
                '/var/www/html/glpi/config/config_db.php'
            ];
            
            foreach ($config_paths as $config_path) {
                if ($config_path && file_exists($config_path)) {
                    // 使用include_once避免重复包含
                    include_once $config_path;
                    error_log("成功加载配置文件: $config_path");
                    break;
                }
            }
            ...
            if (defined('DB_NAME') && defined('DB_USER')) {
                ...
                if (!$db_connection->connect_error) {
                    ...
                    error_log("数据库连接失败: " . $db_connection->connect_error);
                }
            } else {
                error_log("数据库配置常量未定义");
            }
        } catch (Exception $e) {
            error_log("数据库连接尝试失败: " . $e->getMessage());
        }
        ...
        foreach ($csv_data as $row_index => $row) {
            ...
            try {
                ...
                if ($db_connected) {
                    ...
                    if (insertToDatabaseDirect($db_connection, $table, $data)) {
                        $success_count++;
        ...
        $response = [
            'success' => true,
            'message' => "直接导入完成：成功 $success_count 项，失败 $error_count 项",
    ...
    error_log("直接导入处理器错误: " . $e->getMessage());
 ...
 * 直接数据库插入函数
 */
function insertToDatabaseDirect($db_connection, $table, $data) {
    try {
        // 准备插入数据
        $insert_data = [
            'name' => $data['name'],
        ...
        error_log("直接数据库插入错误: " . $e->getMessage());
        return false;
    }
}
