...
if (isset($_GET['ajax_action']) && $_GET['ajax_action'] === 'get_item') {
    ...
    if ($item->getFromDB($id)) {
        // 解析增强字段
        function parseBlacklistEnhancedField($json_data) {
        ...
        echo json_encode([
            ...
            'data' => [
                ...
                'computers_id' => parseBlacklistEnhancedField($item->fields['computers_id']),
                'users_id' => parseBlacklistEnhancedField($item->fields['users_id']),
                'groups_id' => parseBlacklistEnhancedField($item->fields['groups_id']),
...
echo "<form name='form_add' id='blacklist_modal_form' method='post' action='" . $CFG_GLPI["root_doc"] . "/plugins/softwaremanager/front/blacklist.form.submit.php'>";
...
echo "<button type='submit' name='add' class='btn btn-success'><i class='fas fa-plus'></i> " . __('Add to Blacklist', 'softwaremanager') . "</button>";
...
// GLPI标准筛选组件
echo "<div class='center' style='margin-bottom: 20px;'>";
echo "<form method='get' action='" . $CFG_GLPI["root_doc"] . "/plugins/softwaremanager/front/blacklist.php'>";
...
if (!empty($search)) {
    echo "<td><a href='" . $CFG_GLPI["root_doc"] . "/plugins/softwaremanager/front/blacklist.php' class='vsubmit'>" . __('Reset') . "</a></td>";
} else {
    echo "<td></td>";
}
echo "</tr>";
...
echo "<form name='form_blacklist' method='post' action='" . $CFG_GLPI["root_doc"] . "/plugins/softwaremanager/front/blacklist.php'>";
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Blacklist form AJAX handler loaded - v2.1'); // Debug log

    const form = document.querySelector('#blacklist_modal_form');
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // 使用AJAX提交
            const ajaxUrl = '<?php echo $CFG_GLPI['root_doc']; ?>/plugins/softwaremanager/front/blacklist.form.submit.php';
