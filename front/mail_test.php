...
if (isset($_REQUEST['to'])) {
   ...
   if ($body === '') {
      $body = '<p>This is a test mail sent by Softwaremanager plugin using GLPI mail transport.</p>'
            . '<p>Timestamp: '.htmlspecialchars(date('Y-m-d H:i:s')).'</p>';
   }

   if (!filter_var($to, FILTER_VALIDATE_EMAIL)) {
      $result_msg  = __('Invalid email address', 'softwaremanager').': '.htmlspecialchars($raw);
      $result_type = ERROR;
   } else {
      ...
      try {
         ...
         $sent = $mailer->send();
         if ($sent) {
            $result_msg  = sprintf(__('Mail sent to %s', 'softwaremanager'), htmlspecialchars($to));
            $result_type = INFO;
         } else {
            $result_msg  = __('Send failed', 'softwaremanager').': '.htmlspecialchars($mailer->ErrorInfo ?? '');
            $result_type = ERROR;
         }
      } catch (Throwable $e) {
         $result_msg  = __('Send failed', 'softwaremanager').': '.htmlspecialchars($e->getMessage());
         $result_type = ERROR;
      }
   }
