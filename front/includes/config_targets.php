...
// Fetch entities and groups for selects - GLPI 11.x: use doQuery instead of query
$entities = [];
$res = $DB->doQuery("SELECT id, name FROM glpi_entities ORDER BY name");
if ($res) {
   while ($row = $res->fetch_assoc()) { $entities[(int)$row['id']] = $row['name']; }
}
$groups = [];
$res = $DB->doQuery("SELECT g.id, g.name, e.name AS ename FROM glpi_groups g LEFT JOIN glpi_entities e ON e.id=g.entities_id ORDER BY e.name, g.name");
if ($res) {
   while ($row = $res->fetch_assoc()) { $groups[(int)$row['id']] = $row['name'] . ' [' . ($row['ename'] ?? '') . ']'; }
}
...
if ($edit_row) {
   ...
   echo " <a class='vsubmit' href='" . htmlspecialchars($_SERVER['PHP_SELF']) . "#sm-target-list'>".__('Cancel')."</a>";
}
echo "</div>";
...
foreach ($entities as $eid => $ename) {
   ...
   echo "<option value='".(int)$eid."' $sel>" . htmlspecialchars($ename) . "</option>";
}
echo "</select></td></tr>";
...
echo Html::hidden('target_groups_json', ['id' => 'sm-target-groups-json', 'value' => htmlspecialchars($saved_tgj)]);
...
foreach ($groups as $gid => $gname) {
   ...
   echo "<div class='sm-option-item $sel' data-type='target-group' data-value='".(int)$gid."' data-label='".htmlspecialchars($gname)."'>".htmlspecialchars($gname)."</div>";
}
echo "  </div>";
...
$u_rs = $DB->doQuery("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200");
if ($u_rs) { 
    while ($u = $u_rs->fetch_assoc()) { 
        $sel = in_array((int)$u['id'],$pre_users,true) ? 'selected' : '';
        echo "<div class='sm-option-item $sel' data-type='user' data-value='".(int)$u['id']."' data-label='".htmlspecialchars($u['label'])."'>".htmlspecialchars($u['label'])."</div>";
    } 
}
echo "  </div>";
...
foreach ($groups as $gid => $gname) { 
    ...
    echo "<div class='sm-option-item $sel' data-type='group' data-value='".(int)$gid."' data-label='".htmlspecialchars($gname)."'>".htmlspecialchars($gname)."</div>";
}
echo "  </div>";
...
$p_rs = $DB->doQuery("SELECT id, name FROM glpi_profiles ORDER BY name");
if ($p_rs) { 
    while ($p = $p_rs->fetch_assoc()) { 
        $sel = in_array((int)$p['id'],$pre_profiles,true) ? 'selected' : '';
        echo "<div class='sm-option-item $sel' data-type='profile' data-value='".(int)$p['id']."' data-label='".htmlspecialchars($p['name'])."'>".htmlspecialchars($p['name'])."</div>";
    } 
}
echo "  </div>";
...
$pre_emails_str = !empty($pre_emails) ? htmlspecialchars(implode(', ', $pre_emails)) : '';
...
$u_rs2 = $DB->doQuery("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200");
if ($u_rs2) { 
    while ($u = $u_rs2->fetch_assoc()) { 
        $sel = in_array((int)$u['id'],$pre_users,true) ? 'selected' : '';
        echo "<option value='".(int)$u['id']."' $sel>".htmlspecialchars($u['label'])."</option>";
    } 
}
echo "</select>";
...
$p_rs2 = $DB->doQuery("SELECT id, name FROM glpi_profiles ORDER BY name");
if ($p_rs2) { 
    while ($p = $p_rs2->fetch_assoc()) { 
        $sel = in_array((int)$p['id'],$pre_profiles,true) ? 'selected' : '';
        echo "<option value='".(int)$p['id']."' $sel>".htmlspecialchars($p['name'])."</option>";
    } 
}
echo "</select>";
$rec_val = $edit_row ? htmlspecialchars($edit_row['recipients_json']) : '{"users":[],"groups":[],"profiles":[],"emails":[]}';
...
echo "</div>";
echo "<textarea id='sm-options-json' name='options_json' class='sm-hidden'>".htmlspecialchars(json_encode($opt_pref, JSON_UNESCAPED_UNICODE))."</textarea>";
echo "</td></tr>";
...
foreach ($entities as $eid => $ename) {
   echo "<option value='" . (int)$eid . "'>" . htmlspecialchars($ename) . "</option>";
}
echo "</select>";
echo "</div>";
...
if (!empty($rows)) {
   foreach ($rows as $r) {
      ...
      if ($tgj !== '') {
         ...
         if (is_array($tgarr)) {
            foreach ($tgarr as $tid) {
               ...
               if ($tname && $tname !== '&nbsp;') { $tglist[] = '<span class="sm-pill">'.htmlspecialchars($tname).'</span>'; }
            }
         }
      }
      ...
      echo "<tr class='sm-list-row' data-entity-id='" . $eid . "' data-status='" . (int)$r['is_active'] . "' data-search='" . htmlspecialchars($searchData) . "'>";
      echo "<td>" . (int)$r['id'] . "</td>";
      echo "<td>" . htmlspecialchars($ename) . "</td>";
      echo "<td>" . htmlspecialchars($gname) . $targetPills . "</td>";
      ...
      // Try multiple methods to get user names
            ...
            if (!empty($nameList)) {
               $title = htmlspecialchars(implode(' | ', $titleParts));
                $short = htmlspecialchars(call_user_func($shorten, $nameList, 2));
      ...
      $gname = $groups[$gid2] ?? ('#'.$gid2);
            ...
            $uids2 = [];
            $rg = $DB->doQuery("SELECT users_id FROM glpi_groups_users WHERE groups_id=".(int)$gid2);
            if ($rg) { while ($gr = $rg->fetch_assoc()) { $uids2[] = (int)$gr['users_id']; } }
            $uids2 = array_values(array_unique(array_filter($uids2)));

            $emails = [];
            if (!empty($uids2)) {
               $in2 = implode(',', $uids2);
               $ru = $DB->doQuery("SELECT email FROM glpi_users WHERE id IN ($in2)");
               if ($ru) { while ($u = $ru->fetch_assoc()) { $em = trim((string)($u['email'] ?? '')); if ($em !== '') { $emails[$em] = true; } } }
               $re = $DB->doQuery("SELECT email FROM glpi_useremails WHERE users_id IN ($in2)");
               if ($re) { while ($ue = $re->fetch_assoc()) { $em = trim((string)($ue['email'] ?? '')); if ($em !== '') { $emails[$em] = true; } } }
            }
            ...
            $title = htmlspecialchars(($gname ?: 'Group').': '.($preview !== '' ? $preview : '无邮箱'));
            $segments[] = '<span class="sm-pill" data-title="'.$title.'">群组: '.htmlspecialchars($gname).'</span>';
         }
      }

      // Emails
      if (!empty($rec_arr['emails']) && is_array($rec_arr['emails'])) {
         $title = htmlspecialchars(implode(', ', (array)$rec_arr['emails']));
          $short = htmlspecialchars(call_user_func($shorten, (array)$rec_arr['emails'], 2));
      ...
      if (!empty($opt_arr['scope'])) {
         ...
         $option_parts[] = '<strong>范围:</strong> '.($scope_labels[$opt_arr['scope']] ?? htmlspecialchars($opt_arr['scope']));
      }
      ...
      echo "<a class='vsubmit' href='" . htmlspecialchars($_SERVER['PHP_SELF']) . "?edit_id=" . (int)$r['id'] . "' style='margin-right:6px;'><i class='fas fa-edit'></i> " . __('Edit') . "</a>";
