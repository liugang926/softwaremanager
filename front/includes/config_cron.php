...
$printTask = function(string $label, CronTask $t, bool $ok) {
    ...
    echo "<tr class='tab_bg_1'><th style='width:220px;'>" . __('Task name') . "</th><td>" . htmlspecialchars($label) . "</td></tr>";
    if ($ok) {
        // GLPI 11.x: STATE_ENABLE 改为 STATE_WAITING
        $stateMap = [CronTask::STATE_DISABLE => __('Disabled'), CronTask::STATE_WAITING => __('Scheduled'), CronTask::STATE_RUNNING => __('Running')];
        ...
        $comment = htmlspecialchars($t->fields['comment'] ?? '');
        ...
        echo "<tr class='tab_bg_1'><th>" . __('Next run', 'softwaremanager') . "</th><td>" . htmlspecialchars($nextFriendly) . "</td></tr>";
        if (!empty($comment)) {
...
echo "</form>";
// GLPI 11.x: crontask.php 改为 cron.php
echo "<a class='vsubmit' style='margin-right:10px;' href='" . $CFG_GLPI['root_doc'] . "/front/cron.php?itemtype=CronTask&sort=19&order=ASC'>" . __('Open automated actions', 'softwaremanager') . "</a>";
// 使用绝对路径指向插件扫描历史页面
echo "<a class='vsubmit' href='" . $CFG_GLPI['root_doc'] . "/plugins/softwaremanager/front/scanhistory.php'>" . __('Open scan history', 'softwaremanager') . "</a>";
echo "</div>";
