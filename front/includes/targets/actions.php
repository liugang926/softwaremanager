...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'update') {
        ...
        if ($id > 0) {
            $scanId = 0;
            $rs = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id FROM glpi_plugin_softwaremanager_scanhistory WHERE status='completed' ORDER BY id DESC LIMIT 1") : $DB->query("SELECT id FROM glpi_plugin_softwaremanager_scanhistory WHERE status='completed' ORDER BY id DESC LIMIT 1");
            ...
            if ($scanId > 0) {
                ...
                if ($t) {
                    ...
                    $drs = method_exists($DB, 'doQuery') ? $DB->doQuery($q) : $DB->query($q);
                    ...
                    echo "<div><strong>".sprintf(__('Preview for group ID %d (scope=%s)', 'softwaremanager'), $gid, htmlspecialchars($scope))."</strong></div>";
                    ...
                    if (!empty($rows)) {
                        ...
                        foreach ($rows as $r) {
                            ...
                            $label = sprintf('%s (%s) @ %s', htmlspecialchars($r['software_name']), htmlspecialchars($r['software_version']), htmlspecialchars($r['computer_name'] ?? ''));
        ...
        if (($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) && Session::getLoginUserID()) {
            ...
            $qe = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT email FROM glpi_useremails WHERE users_id=".$uid." ORDER BY is_default DESC, id ASC LIMIT 1") : $DB->query("SELECT email FROM glpi_useremails WHERE users_id=".$uid." ORDER BY is_default DESC, id ASC LIMIT 1");
            ...
            if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $qu = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT email FROM glpi_users WHERE id=".$uid." LIMIT 1") : $DB->query("SELECT email FROM glpi_users WHERE id=".$uid." LIMIT 1");
        ...
        if ($id > 0 && filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $type = INFO;
            if ($ok) {
                $qr = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, sendtime, state, errormessage FROM glpi_queuednotifications WHERE recipient LIKE '%".$DB->escape($email)."%' ORDER BY id DESC LIMIT 1") : $DB->query("SELECT id, sendtime, state, errormessage FROM glpi_queuednotifications WHERE recipient LIKE '%".$DB->escape($email)."%' ORDER BY id DESC LIMIT 1");
            ...
            Session::addMessageAfterRedirect(__('Invalid email address', 'softwaremanager') . ': ' . htmlspecialchars($email_raw), false, WARNING);
            $_SESSION['sm_scroll_to_list'] = true;
            Html::redirect($_SERVER['PHP_SELF']);
        }
        ...
        if (empty($emails)) {
            ...
            Session::addMessageAfterRedirect(__('Resolved recipients: ','softwaremanager') . implode(', ', array_map('htmlspecialchars', $emails)), false, INFO);
        }
        $_SESSION['sm_scroll_to_list'] = true;
        Html::redirect($_SERVER['PHP_SELF']);
    }
