...
// Fetch selects
$entities = [];
$res = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, name FROM glpi_entities ORDER BY name") : $DB->query("SELECT id, name FROM glpi_entities ORDER BY name");
if ($res) { while ($row = $DB->fetchAssoc($res)) { $entities[(int)$row['id']] = $row['name']; } }

$groups = [];
$res = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT g.id, g.name, e.name AS ename FROM glpi_groups g LEFT JOIN glpi_entities e ON e.id=g.entities_id ORDER BY e.name, g.name") : $DB->query("SELECT g.id, g.name, e.name AS ename FROM glpi_groups g LEFT JOIN glpi_entities e ON e.id=g.entities_id ORDER BY e.name, g.name");
...
$u_rs = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200") : $DB->query("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200");
if ($u_rs) { while ($u = $DB->fetchAssoc($u_rs)) { $sel = in_array((int)$u['id'],$pre_users,true) ? 'selected' : ''; echo "<div class='sm-option-item $sel' data-type='user' data-value='".(int)$u['id']."' data-label='".htmlspecialchars($u['label'])."'>".htmlspecialchars($u['label'])."</div>"; } }
echo "  </div>";
...
foreach ($groups as $gid => $gname) { $sel = in_array((int)$gid,$pre_groups,true) ? 'selected' : ''; echo "<div class='sm-option-item $sel' data-type='group' data-value='".(int)$gid."' data-label='".htmlspecialchars($gname)."'>".htmlspecialchars($gname)."</div>"; }
echo "  </div>";
...
$p_rs = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, name FROM glpi_profiles ORDER BY name") : $DB->query("SELECT id, name FROM glpi_profiles ORDER BY name");
if ($p_rs) { while ($p = $DB->fetchAssoc($p_rs)) { $sel = in_array((int)$p['id'],$pre_profiles,true) ? 'selected' : ''; echo "<div class='sm-option-item $sel' data-type='profile' data-value='".(int)$p['id']."' data-label='".htmlspecialchars($p['name'])."'>".htmlspecialchars($p['name'])."</div>"; } }
echo "  </div>";
...
$u_rs2 = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200") : $DB->query("SELECT id, CONCAT(IFNULL(realname,''),' ',IFNULL(firstname,''),' (',name,')') AS label FROM glpi_users ORDER BY realname, firstname, name LIMIT 200");
if ($u_rs2) { while ($u = $DB->fetchAssoc($u_rs2)) { $sel = in_array((int)$u['id'],$pre_users,true) ? 'selected' : ''; echo "<option value='".(int)$u['id']."' $sel>".htmlspecialchars($u['label'])."</option>"; } }
echo "</select>";
echo "<select id='sm-groups' class='sm-hidden' multiple>";
foreach ($groups as $gid => $gname) { $sel = in_array((int)$gid,$pre_groups,true) ? 'selected' : ''; echo "<option value='".(int)$gid."' $sel>".htmlspecialchars($gname)."</option>"; }
echo "</select>";
echo "<select id='sm-profiles' class='sm-hidden' multiple>";
$p_rs2 = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT id, name FROM glpi_profiles ORDER BY name") : $DB->query("SELECT id, name FROM glpi_profiles ORDER BY name");
if ($p_rs2) { while ($p = $DB->fetchAssoc($p_rs2)) { $sel = in_array((int)$p['id'],$pre_profiles,true) ? 'selected' : ''; echo "<option value='".(int)$p['id']."' $sel>".htmlspecialchars($p['name'])."</option>"; } }
echo "</select>";
...
if ($edit_row) { echo " <a class='vsubmit' href='" . htmlspecialchars($_SERVER['PHP_SELF']) . "#sm-target-list'>" . __('Cancel') . "</a>"; }
echo "</td></tr>";
echo "  </table>";
echo "  </form>";
echo " </div>";
