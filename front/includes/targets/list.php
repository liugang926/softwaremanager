...
// Total count
$totalCount = 0;
$rsCount = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT COUNT(*) AS cnt FROM glpi_plugin_softwaremanager_group_mail_targets") : $DB->query("SELECT COUNT(*) AS cnt FROM glpi_plugin_softwaremanager_group_mail_targets");
...
foreach ($entities as $eid => $ename) { echo "<option value='".(int)$eid."'>".htmlspecialchars($ename)."</option>"; }
echo "</select></div>";
    ...
    foreach ($rows as $r) {
        ...
        if (!empty($rec_arr['groups']) && is_array($rec_arr['groups'])) {
            ...
            foreach ($gids as $gid2) {
                ...
                $uids2 = [];
                $rg = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT users_id FROM glpi_groups_users WHERE groups_id=".(int)$gid2) : $DB->query("SELECT users_id FROM glpi_groups_users WHERE groups_id=".(int)$gid2);
                if ($rg) { while ($gr = $DB->fetchAssoc($rg)) { $uids2[] = (int)$gr['users_id']; } }
                $uids2 = array_values(array_unique(array_filter($uids2)));
                $emails = [];
                if (!empty($uids2)) {
                    $in2 = implode(',', $uids2);
                    $ru = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT email FROM glpi_users WHERE id IN ($in2)") : $DB->query("SELECT email FROM glpi_users WHERE id IN ($in2)");
                    if ($ru) { while ($u = $DB->fetchAssoc($ru)) { $em = trim((string)($u['email'] ?? '')); if ($em !== '') { $emails[$em] = true; } } }
                    $re = method_exists($DB, 'doQuery') ? $DB->doQuery("SELECT email FROM glpi_useremails WHERE users_id IN ($in2)") : $DB->query("SELECT email FROM glpi_useremails WHERE users_id IN ($in2)");
                ...
                $title = htmlspecialchars(($gname2 ?: 'Group').': '.($preview !== '' ? $preview : '无邮箱'));
                $segments[] = '<span class="sm-pill" data-title="'.$title.'">群组: '.htmlspecialchars($gname2).'</span>';
            }
        }
        ...
        if (!empty($opt_arr['scope'])) { $scope_labels = ['main'=>'主要','tech'=>'技术','both'=>'两种']; $option_parts[] = '<strong>范围:</strong> '.($scope_labels[$opt_arr['scope']] ?? htmlspecialchars($opt_arr['scope'])); }
