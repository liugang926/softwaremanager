...
if (isset($_POST["delete_single"]) && isset($_POST["item_id"])) {
    ...
    if ($scan_id > 0) {
        global $DB;
        // Use delete() for GLPI 11.x compatibility
        $DB->delete('glpi_plugin_softwaremanager_scanhistory', ['id' => $scan_id]);
...
// Get scan history data using doQuery for GLPI 11.x compatibility
global $DB;
...
// Embed CSRF token for AJAX requests
var SOFTWAREMANAGER_CSRF_TOKEN = '<?php echo Session::getNewCSRFToken(); ?>';
...
function startComplianceScan() {
    ...
    formData.append('_glpi_csrf_token', SOFTWAREMANAGER_CSRF_TOKEN);

    // Start the scan - use front/ directory path for GLPI 11.x compatibility
    fetch('<?php echo $CFG_GLPI['root_doc']; ?>/plugins/softwaremanager/front/ajax_compliance_scan.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
