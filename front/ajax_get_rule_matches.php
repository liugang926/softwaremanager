...
try {
    ...
    // 获取 exact_match 字段值
    $exact_match = intval($rule_data['exact_match'] ?? 0);

    // 基于规则名称进行软件名称匹配，支持通配符和精确匹配
    ...
    if (!empty($software_name)) {
        try {
            ...
            if ($software_name === '*') {
                ...
                $safe_pattern = addslashes($like_pattern);
                $where_condition = "s.name LIKE '{$safe_pattern}'";
            } else {
                // 根据 exact_match 字段决定匹配方式
                // 注意：对于LIKE模式，需要使用不同的转义方式
                if ($exact_match == 1) {
                    // 精确匹配：软件名称必须完全相同
                    $safe_software_name = addslashes($software_name);
                    $where_condition = "s.name = '{$safe_software_name}'";
                } else {
                    // 部分匹配：软件名称包含规则名称（默认行为）
                    // 对于LIKE，只转义单引号，保留%通配符
                    $safe_software_name = addslashes($software_name);
                ...
                if (!empty($version_rules)) {
                    ...
                    foreach ($version_rules as $rule) {
                        ...
                        if (strpos($rule, '>=') !== false) {
                            ...
                            $single_version_condition = "sv.name >= '" . addslashes($version) . "'";
                            ...
                            if (count($range) == 2) {
                                ...
                                $single_version_condition = "sv.name >= '" . addslashes($min_version) . "' AND sv.name <= '" . addslashes($max_version) . "'";
                            }
                        } else {
                            // 精确匹配
                            $single_version_condition = "sv.name = '" . addslashes($rule) . "'";
                        }
            ...
            // 统计查询 - 使用原始SQL查询（与诊断脚本相同的方法）
            $count_sql = "SELECT
                COUNT(DISTINCT isv.id) as total_count,
            ...
            if ($stats['total_installations'] > 0) {
                ...
                $detail_result = $DB->doQuery($detail_sql);
    ...
    $response = [
        ...
        'debug' => [
            ...
            'exact_match_label' => $exact_match == 1 ? '精确匹配' : '部分匹配',
