...
// Check rights - using haveRight to avoid 403 error on POST requests
...
// Add CSS and JS directly for GLPI 11.x compatibility
$plugin_path = $CFG_GLPI['root_doc'] . '/plugins/softwaremanager/public';
echo "<link rel='stylesheet' type='text/css' href='{$plugin_path}/css/softwaremanager.css'>";
echo "<script type='text/javascript' src='{$plugin_path}/js/softwaremanager.js'></script>";
...
function showSoftwareDetails(softwareId) {
    ...
    // 构建 AJAX URL - 与 blacklist.php 相同的方式
    const ajaxUrl = '<?php echo $CFG_GLPI["root_doc"]; ?>/plugins/softwaremanager/front/ajax_software_details.php?software_id=' + softwareId;
...
function createComputerPreview(element, softwareId, softwareName) {
    ...
    // 构建 AJAX URL - 与 blacklist.php 相同的方式
    const ajaxUrl = '<?php echo $CFG_GLPI["root_doc"]; ?>/plugins/softwaremanager/front/ajax_software_details.php?software_id=' + softwareId;
    console.log('createComputerPreview ajaxUrl:', ajaxUrl);
    ...
    fetch(ajaxUrl)
        ...
        .then(data => {
            ...
            if (data.success && data.computers) {
                ...
                computerPreviewElement.innerHTML = '<div class="error">Error: ' + (data.error || 'Unknown error') + '</div>';
                if (data.debug) {
                    computerPreviewElement.innerHTML += '<br><small>Debug: ' + JSON.stringify(data.debug) + '</small>';
                }
            }
        })
        .catch(error => {
...
function performBatchAction(action) {
    ...
    if (confirm('Are you sure you want to add ' + checkedBoxes.length + ' selected software items to the ' + actionText + '?')) {
        // 使用AJAX端点进行批量操作，避免GLPI 11.x路由问题
        var ajaxUrl = '<?php echo $CFG_GLPI['root_doc']; ?>/plugins/softwaremanager/front/ajax_add_to_list.php';

        // 收集所有选中的软件名称
        var softwareNames = [];
        checkedBoxes.forEach(function(checkbox) {
            softwareNames.push(checkbox.value);
        });

        var formData = new FormData();
        formData.append('action', action);
        softwareNames.forEach(function(name) {
            formData.append('software_names[]', name);
        });
        formData.append('_glpi_csrf_token', document.querySelector('input[name="_glpi_csrf_token"]').value);
...
// 处理单个软件添加到白名单/黑名单的函数 - 使用AJAX端点避免GLPI 11.x路由问题
function addSingleToList(softwareName, action) {
    ...
    if (confirm('Are you sure you want to add "' + softwareName + '" to the ' + actionText + '?')) {
        // 使用专门的AJAX端点，避免GLPI 11.x的Symfony路由拦截
        var ajaxUrl = '<?php echo $CFG_GLPI['root_doc']; ?>/plugins/softwaremanager/front/ajax_add_to_list.php';
...
// 处理状态移除功能 - 使用AJAX端点避免GLPI 11.x路由问题
function removeFromStatus(softwareName, statusType) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert('Error removing software: ' + (data.error || 'Unknown error'));
            // 恢复按钮状态
            window.location.reload();
        }
    })
