...
if ($selected_software) {
    ...
    echo "<h4>版本分布分析</h4>";
    $versions = [];
    $sql = "SELECT software_version, compliance_status, COUNT(*) as count,
                      COUNT(DISTINCT computer_name) as computer_count
                      FROM `glpi_plugin_softwaremanager_scandetails`
                      WHERE scanhistory_id = $scanhistory_id AND software_name = '" . addslashes($selected_software) . "'
    ...
    $details_rows = [];
    $sql = "SELECT * FROM `glpi_plugin_softwaremanager_scandetails`
                      WHERE scanhistory_id = $scanhistory_id AND software_name = '" . addslashes($selected_software) . "'
    ...
    $total_software = 0;
    $sql = "SELECT COUNT(DISTINCT software_name) as count
            FROM `glpi_plugin_softwaremanager_scandetails`
            WHERE scanhistory_id = $scanhistory_id";
    $result = $DB->doQuery($sql);
    if ($result && ($row = $result->fetch_assoc())) {
        $total_software = $row['count'];
    }
    ...
    $category_stats = ['approved_software' => 0, 'blacklisted_software' => 0, 'unmanaged_software' => 0, 'total_software' => 0];
    $sql = "SELECT
