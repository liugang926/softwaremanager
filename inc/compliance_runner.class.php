...
class PluginSoftwaremanagerComplianceRunner {
   ...
   public static function run(): array {
      ...
      PluginSoftwaremanagerLogger::info('Compliance scan started');
      ...
      $scan_id = 0;
      $db_error = '';
      $insert_result = $DB->insert('glpi_plugin_softwaremanager_scanhistory', [
         'user_id' => $user_id,
         'scan_date' => $scan_time,
         'total_software' => $total,
         'whitelist_count' => $approved_count,
         'blacklist_count' => $black_count,
      ...
      if ($insert_result === false) {
         ...
         $db_error = $DB->error();
         PluginSoftwaremanagerLogger::error('Insert scan history failed', ['error' => $db_error]);
      } else {
      ...
      PluginSoftwaremanagerLogger::info('Compliance scan completed', [
         'total' => $total,
   ...
   private static function fetchRules(string $table): array {
      ...
      if ($DB->tableExists($table)) {
         $sql = "SELECT id, name, version, computers_id, users_id, groups_id, version_rules, computer_required, user_required, group_required, version_required, comment FROM `$table` WHERE is_active = 1";
         $result = $DB->doQuery($sql);
         if ($result) {
            while ($row = $result->fetch_assoc()) {
               $rules[] = $row;
