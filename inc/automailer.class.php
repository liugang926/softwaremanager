...
class PluginSoftwaremanagerAutomailer {
   ...
   public static function cronSoftwaremanager_autoscan_mailer(CronTask $task): int {
      global $DB;
      try {
         PluginSoftwaremanagerLogger::info('Cron mailer task started');
         if (!$DB) {
         ...
         $latest = null;
         $sql = "SELECT id, scan_date, total_software, whitelist_count, blacklist_count, unmanaged_count, status FROM glpi_plugin_softwaremanager_scanhistory WHERE status='completed' ORDER BY id DESC LIMIT 1";
         ...
         $details = [];
         $sql = "SELECT d.id, d.software_name, d.software_version, d.computer_id, d.computer_name, d.computer_serial,
         ...
         $targets = [];
         foreach ($DB->request(['FROM' => 'glpi_plugin_softwaremanager_group_mail_targets', 'WHERE' => ['is_active' => 1], 'ORDER' => ['entities_id', 'groups_id']]) as $row) {
            $targets[] = $row;
         }
         ...
         if (!empty($byComputer)) {
            foreach ($byComputer as $comp) {
               ...
               $owners = [];
               $primaryName = '';
               foreach ($DB->request(['SELECT' => ['users_id', 'users_id_tech'], 'FROM' => 'glpi_computers', 'WHERE' => ['id' => $cid], 'LIMIT' => 1]) as $cu) {
                  $owners[] = (int)($cu['users_id'] ?? 0);
                  $owners[] = (int)($cu['users_id_tech'] ?? 0);
                  // Try to fetch primary owner name
                  foreach ($DB->request(['SELECT' => ['realname', 'name'], 'FROM' => 'glpi_users', 'WHERE' => ['id' => (int)($cu['users_id'] ?? 0)], 'LIMIT' => 1]) as $un) {
                     $primaryName = (string)($un['realname'] ?? $un['name'] ?? '');
                     break;
                  }
                  break;
               }
               ...
               $sql = "SELECT os.name AS osname, osv.name AS ver, ios.installation_date
               ...
               $result = $DB->doQuery($sql);
               if ($result && ($os = $result->fetch_assoc())) {
               ...
               if ($osVersion === '' && $osInstall === '') {
                  foreach ($DB->request(['SELECT' => ['operatingsystems_name', 'operatingsystems_version', 'os_install_date', 'date_creation'], 'FROM' => 'glpi_computers', 'WHERE' => ['id' => $cid], 'LIMIT' => 1]) as $os2) {
         ...
         PluginSoftwaremanagerLogger::info('Cron mailer task completed', ['prepared' => $preparedCount]);
         return 1;
      } catch (Throwable $e) {
         PluginSoftwaremanagerLogger::error('Cron mailer failed', ['exception' => $e->getMessage()]);
   ...
   public static function resolveRecipientEmails(array $spec): array {
      ...
      if (!empty($spec['users']) && is_array($spec['users'])) {
         ...
         if (!empty($ids)) {
            foreach ($DB->request(['SELECT' => ['id', 'email'], 'FROM' => 'glpi_users', 'WHERE' => ['id' => $ids]]) as $u) {
               $em = trim((string)($u['email'] ?? ''));
               if ($em !== '') $emails[$em] = true;
            }
            // fallback: glpi_useremails
            foreach ($DB->request(['SELECT' => ['users_id', 'email'], 'FROM' => 'glpi_useremails', 'WHERE' => ['users_id' => $ids]]) as $ue) {
      ...
      if (!empty($spec['groups']) && is_array($spec['groups'])) {
         ...
         if (!empty($gids)) {
            $uids = [];
            foreach ($DB->request(['SELECT' => ['DISTINCT' => 'gu.users_id'], 'FROM' => 'glpi_groups_users gu', 'WHERE' => ['gu.groups_id' => $gids]]) as $r) {
               $uids[] = (int)$r['users_id'];
            }
            if (!empty($uids)) {
               $uids = array_values(array_unique($uids));
               foreach ($DB->request(['SELECT' => ['id', 'email'], 'FROM' => 'glpi_users', 'WHERE' => ['id' => $uids]]) as $u) {
                  $em = trim((string)($u['email'] ?? ''));
                  if ($em !== '') $emails[$em] = true;
               }
               foreach ($DB->request(['SELECT' => ['users_id', 'email'], 'FROM' => 'glpi_useremails', 'WHERE' => ['users_id' => $uids]]) as $ue) {
                  $em = trim((string)($ue['email'] ?? ''));
                  if ($em !== '') $emails[$em] = true;
               }
            }
      ...
      if (!empty($spec['profiles']) && is_array($spec['profiles'])) {
         ...
         if (!empty($pids)) {
            $uids = [];
            foreach ($DB->request(['SELECT' => ['DISTINCT' => 'users_id'], 'FROM' => 'glpi_profiles_users', 'WHERE' => ['profiles_id' => $pids]]) as $r) {
               $uids[] = (int)$r['users_id'];
            }
            if (!empty($uids)) {
               $uids = array_values(array_unique($uids));
   ...
   private static function enqueueNotification(string $event, array $data, array $emails): array {
      ...
      // Provide a subject fallback if template has none - 生成有意义的默认主题
      if (empty($options['subject'])) {
         $options['subject'] = self::generateDefaultSubject($event, $options);
      }
      ...
      global $DB;
       if ($DB) {
          $sql = "SELECT id FROM glpi_queuednotifications WHERE 1=0";
          $result = $DB->doQuery($sql);
          if ($result) {
             while ($r = $result->fetch_assoc()) {
                $queuedIds[] = (int)$r['id'];
   ...
   public static function sendTestForTarget(int $targetId, string $email): bool {
      ...
      $scanId = 0;
      $sql = "SELECT id FROM glpi_plugin_softwaremanager_scanhistory WHERE status='completed' ORDER BY id DESC LIMIT 1";
      $result = $DB->doQuery($sql);
      ...
      // Load details
      $details = [];
      $sql = "SELECT d.id, d.software_name, d.software_version, d.computer_id, d.computer_name, d.compliance_status,
   ...
   private static function getGroupName(int $groupId): string {
      global $DB;
      foreach ($DB->request(['SELECT' => ['name'], 'FROM' => 'glpi_groups', 'WHERE' => ['id' => $groupId], 'LIMIT' => 1]) as $r) {
   ...
   /**
    * 生成默认邮件主题 - 用于没有提供主题时的回退
    */
   private static function generateDefaultSubject(string $event, array $options): string {
      $reportDate = date('Y-m-d');
      $scanId = (string)($options['scan_id'] ?? '');
      $dateSuffix = ' | 日期 ' . $reportDate;
      $idSuffix = $scanId !== '' ? ' | ID #' . $scanId : '';

      switch ($event) {
         case 'softwaremanager_group_report':
            $groupName = (string)($options['group_name'] ?? '');
            $baseSubject = '[GLPI] 群组合规报告';
            if ($groupName !== '') {
               $baseSubject .= ' - ' . $groupName;
            }
            return $baseSubject . $idSuffix . $dateSuffix;

         case 'softwaremanager_computer_report':
            $computerName = (string)($options['computer_name'] ?? '');
         ...
         default:
            return '[GLPI] 软件合规报告' . $idSuffix . $dateSuffix;
      }
   }
}
