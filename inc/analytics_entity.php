...
if ($details_table_exists) {
    $details_count = 0;
    foreach ($DB->request(['COUNT' => 'cpt', 'FROM' => 'glpi_plugin_softwaremanager_scandetails', 'WHERE' => ['scanhistory_id' => $scanhistory_id]]) as $row) {
        $details_count = $row['cpt'];
        break;
    }
...
if ($selected_entity_id) {
    $entity_name = "实体 #$selected_entity_id";
    foreach ($DB->request(['SELECT' => ['name'], 'FROM' => 'glpi_entities', 'WHERE' => ['id' => $selected_entity_id]]) as $row) {
...
if ($selected_entity_id) {
    ...
    $risk_stats = ['safe_entities' => 0, 'medium_risk_entities' => 0, 'high_risk_entities' => 0, 'total_entities' => 0];
    $sql = "SELECT
             COUNT(DISTINCT CASE WHEN blacklisted_count = 0 THEN id END) as safe_entities,
             COUNT(DISTINCT CASE WHEN blacklisted_count BETWEEN 1 AND 10 THEN id END) as medium_risk_entities,
             COUNT(DISTINCT CASE WHEN blacklisted_count > 10 THEN id END) as high_risk_entities,
