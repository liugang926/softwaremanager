...
class PluginSoftwaremanagerRule extends CommonDBTM
{
    ...
    public function matchSoftware($software_record) {
        ...
        $rules = [];
        foreach ($DB->request(['FROM' => self::getTable(), 'WHERE' => ['is_active' => 1, 'is_deleted' => 0], 'ORDER' => ['priority' => 'DESC', 'id' => 'ASC']]) as $row) {
            $rules[] = $row;
        }
    ...
    static function migrateExistingRules() {
        ...
        if ($DB->tableExists($whitelist_table)) {
            foreach ($DB->request(['FROM' => $whitelist_table, 'WHERE' => ['is_deleted' => 0]]) as $row) {
                $rule = new self();
                $rule_data = [
        ...
        if ($DB->tableExists($blacklist_table)) {
            foreach ($DB->request(['FROM' => $blacklist_table, 'WHERE' => ['is_deleted' => 0]]) as $row) {
                $rule = new self();
                $rule_data = [
                    'name' => 'Migrated: ' . $row['name'],
                    'pattern' => $row['name'],
