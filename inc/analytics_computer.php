...
if ($selected_computer) {
    ...
    $total_computers = 0;
    $sql = "SELECT COUNT(DISTINCT computer_name) as count
            FROM `glpi_plugin_softwaremanager_scandetails`
            WHERE scanhistory_id = $scanhistory_id";
    $result = $DB->doQuery($sql);
    if ($result && ($row = $result->fetch_assoc())) {
        $total_computers = $row['count'];
    }
    ...
    $risk_stats = ['safe_computers' => 0, 'medium_risk_computers' => 0, 'high_risk_computers' => 0, 'total_computers' => 0];
    $sql = "SELECT
